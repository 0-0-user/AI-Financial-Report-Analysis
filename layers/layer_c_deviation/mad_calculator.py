...
def pettitt_test(values: list[float]) -> tuple[float, int]:
    ...
    # 近似 p-value (Pettitt 1979, eq 2.16): p ≈ 2 × exp(-6K²/(n³+n²))
...
def run_deviation_analysis(
    ...
    for indicator, peer_median in benchmark.peer_median.items():
        ...
        logger.debug(f"{indicator}: actual={actual} median={median} "
                     f"scale={scale:.4f}({method}) mult={multiple:.1f}× [{severity}]")
